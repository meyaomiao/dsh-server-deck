...
var HostPool = class {
  constructor(resolveHost2, resolveSecret) {
    this.resolveHost = resolveHost2;
   ...
   * /bin/sh 不存在时再试 sh -s。
   */
  execSh(id, script, timeoutMs = 2e4) {
...
`;
  ...
  runExec(id, command, timeoutMs, stdin) {
...
function normalizeNetMonth(raw) {
  if (raw === null || typeof raw !== "object") return void 0;
  const o = raw;
  if (typeof o.ym !== "string" || !/^\d{4}-\d{2}$/.test(o.ym)) return void 0;
  const rxBytes = Number(o.rxBytes);
  const txBytes = Number(o.txBytes);
  if (!Number.isFinite(rxBytes) || !Number.isFinite(txBytes) || rxBytes < 0 || txBytes < 0) return void 0;
  const state = { ym: o.ym, rxBytes, txBytes };
...
function accumulateNetMonth(prev, now, rx, tx) {
  const ym = monthKey(now);
  if (rx === void 0 && tx === void 0) {
    if (prev !== void 0 && prev.ym === ym) return prev;
    return { ym, rxBytes: 0, txBytes: 0 };
  }
  const rxBytes = prev !== void 0 && prev.ym === ym ? prev.rxBytes : 0;
  const txBytes = prev !== void 0 && prev.ym === ym ? prev.txBytes : 0;
  let rxAdd = 0;
  let txAdd = 0;
  if (rx !== void 0 && prev?.lastRx !== void 0 && rx >= prev.lastRx) rxAdd = rx - prev.lastRx;
  if (tx !== void 0 && prev?.lastTx !== void 0 && tx >= prev.lastTx) txAdd = tx - prev.lastTx;
  return {
    ym,
...
var MetricStore = class {
  ...
  async load() {
    ...
    for (const id of hostIds) {
      ...
      try {
        const raw2 = JSON.parse(await readFile4(join3(this.rootDir, id, "net-month.json"), "utf8"));
        const state = normalizeNetMonth(raw2);
        if (state !== void 0) this.netMonth.set(id, state);
      } catch {
      }
    }
  }
   ...
   * 无计数器时只刷新月键(换月清零)。
   */
  async applyNetSample(hostId, now, rx, tx) {
    ...
    await this.withLock(hostId, async () => {
      ...
      await atomicWrite2(join3(this.rootDir, hostId, "net-month.json"), `${JSON.stringify(next)}
`);
    });
    return next;
  }
...
function hasUsefulMetrics(parsed) {
  return parsed.cpuPercent !== void 0 || parsed.memPercent !== void 0;
}
...
function parseCpTime(cpuSection) {
  ...
  if (nums.length < 10 || nums.some((n) => !Number.isFinite(n))) return void 0;
...
function parseWinLoad(cpuSection) {
  const m = /winload=(\d+)/.exec(cpuSection);
  if (m === null) return void 0;
  const n = Number(m[1]);
  return Number.isFinite(n) ? pctClamp(n) : void 0;
}
function parsePhysmem(memSection) {
...
function isCountedNetIface(name2) {
  const n = name2.trim().replace(/:$/, "").toLowerCase();
  if (n.length === 0) return false;
  if (n.includes("loopback") || n.includes("isatap") || n.includes("teredo") || n.includes("wan miniport") || n.includes("bluetooth") || n.includes("pseudo") || n.startsWith("docker") || n.startsWith("veth") && !n.startsWith("vethernet")) {
    return false;
  }
...
function rateBps(a, b, dtSec) {
  if (!(dtSec > 0) || b < a) return void 0;
  return Math.round((b - a) / dtSec);
}
function applyNet(result, text) {
  ...
  if (n1 !== void 0 && n2 !== void 0) {
    ...
    if (rx !== void 0) result.netRxBps = rx;
    if (tx !== void 0) result.netTxBps = tx;
  }
}
function parseProbeOutput(raw) {
  ...
  else if (unameLine !== void 0 && /^(Windows|MINGW|MSYS|CYGWIN)/.test(unameLine)) result.osName = "Windows";
  ...
  if (procUp !== null && !/\bup\s+/i.test(upSection)) {
    ...
    if (formatted !== void 0) result.uptimeText = formatted;
  } else {
  ...
  } else if (cpTimeCpu !== void 0) {
    result.cpuPercent = cpTimeCpu;
  } else if (winCpu !== void 0) {
  ...
  if (memTotal !== void 0 && memTotal > 0 && memAvail !== void 0) {
    result.memPercent = pctClamp((memTotal - memAvail) / memTotal * 100);
  } else if (memTotal !== void 0 && memTotal > 0) {
...
var MetricRecorder = class {
  ...
  async probeOne(id) {
    ...
    try {
      ...
      const month = await this.metrics.applyNetSample(id, now, parsed.netRxBytes, parsed.netTxBytes).catch(() => accumulateNetMonth(prevNet, now, parsed.netRxBytes, parsed.netTxBytes));
      let netRxBps = parsed.netRxBps;
      let netTxBps = parsed.netTxBps;
      const dtSec = prevNet?.lastT !== void 0 ? (now - prevNet.lastT) / 1e3 : 0;
      if (netRxBps === void 0 && parsed.netRxBytes !== void 0 && prevNet?.lastRx !== void 0 && dtSec > 0 && parsed.netRxBytes >= prevNet.lastRx) {
        netRxBps = Math.round((parsed.netRxBytes - prevNet.lastRx) / dtSec);
      }
      if (netTxBps === void 0 && parsed.netTxBytes !== void 0 && prevNet?.lastTx !== void 0 && dtSec > 0 && parsed.netTxBytes >= prevNet.lastTx) {
...
// src/server/exec-tools.ts
import { defineTool } from "@deepseek-ai/dsh-tools";

// src/server/resolve-host.ts
function norm(s) {
...
function resolveHost(hosts, query) {
  const q = query.trim();
  ...
  const byAddr = hosts.filter((h) => {
    ...
    return host === nq || norm(`${h.username}@${h.host}`) === nq || norm(`${h.host}:${String(h.port)}`) === nq;
  });
  if (byAddr.length === 1) return { ok: true, host: byAddr[0] };
  if (byAddr.length > 1) {
    return { ok: false, error: `\u591A\u53F0\u4E3B\u673A\u5339\u914D ${q}: ${byAddr.map(label).join(", ")}` };
  }
  const fuzzy = hosts.filter(
    (h) => norm(h.id).includes(nq) || norm(h.name).includes(nq) || norm(h.host).includes(nq)
  );
...
// src/server/wrap-command.ts
function looksLikeWindows(osName) {
  return osName !== void 0 && /windows/i.test(osName);
}
var SHELL_PREFIX = /^(powershell\.exe|pwsh(?:\.exe)?|cmd(?:\.exe)?)\b/i;
function wrapRemoteCommand(command, osName) {
...
function clampTimeoutMs(raw) {
  const n = raw === void 0 ? 6e4 : Number(raw);
  if (!Number.isFinite(n)) return 6e4;
  return Math.max(1e3, Math.min(3e5, Math.round(n)));
}
var MAX_CAPTURE = 32768;
function truncateCapture(text) {
...
function registerExecTools(ctx, store, pool, recorder) {
  ...
  ctx.tools.register(
    defineTool({
      ...
      description: "List Server Deck ledger hosts (id, name, address, username, tags, last probed OS). No secrets.",
      parameters: {},
      output: {
        schema: {
          type: "object",
        ...
        render: (_args, value) => {
          const lines = value.hosts.map((h) => {
            const tag = h.tags !== void 0 && h.tags.length > 0 ? ` [${h.tags.join(",")}]` : "";
            const os = h.osName !== void 0 ? ` ${h.osName}` : "";
            const st = h.state !== void 0 ? ` ${h.state}` : "";
  ...
  ctx.tools.register(
    defineTool({
      ...
      description: "Run one non-interactive command on a Server Deck host over SSH. Returns exit code, stdout and stderr. Not the card xterm; no secrets in the result.",
      parameters: {
        host: {
          type: "string",
          required: true,
      ...
      output: {
        ...
        render: (_args, value) => {
          const code = value.code === void 0 ? "timeout" : String(value.code);
...
${body}` : head }];
      ...
      async execute(args) {
        ...
        return {
          ...
          code: result.code === null ? void 0 : result.code,
          stdout: out.text,
          stderr: err.text,
...
function apply(ctx) {
  ...
  registerExecTools(ctx, store, pool, recorder);
}
export {
  apply,
  inject,
